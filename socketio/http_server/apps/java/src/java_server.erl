...
%% @end
%%--------------------------------------------------------------------
handle_cast(ping_pong_with_java, State) ->
    io:format("message ping_pong_with_java!! ============== ~n~n"),

    % {ok, JavaMBox} = application:get_env(java, java_mailbox),
    % io:format("message ~p!! ============== ~n~n", [GoMBox]),
    % go_sup:start_name_server(),

    % gen_server:cast(JavaMBox, {ping, self()}),

    net_adm:ping('java_node@127.0.0.1'),
    {noreply, State};
...
handle_info({pong, Pid}, #state{remote_pid = undefined} = State) ->
    % io:format("message pong!! JAVA============== ~n~n"),
